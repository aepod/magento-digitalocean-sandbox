...
if [ ! -f ".vagrantlocks/magentoinstall" ] && [ -f "/usr/local/bin/n98-magerun.phar" ] && [ -f "/root/.n98-magerun.yaml" ]; then
	...
	/usr/local/bin/n98-magerun.phar install -n --dbHost="localhost" --dbUser="magento" --dbPass="5678password" --dbName="magento" --installSampleData=no --useDefaultConfigParams=yes --magentoVersionByName="magento1910" --installationFolder="/var/www/html/" --baseUrl="http://www.idealphp.com/"
